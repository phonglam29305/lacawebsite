...
$.extend(_TMS,{etal:'<div></div>',items:'.items>li',pic:'pic',mask:'mask',paginationCl:'pagination',currCl:'current',pauseCl:'paused',bannerCl:'banner',numStatusCl:'numStatus',pagNums:true,overflow:'hidden',show:0,changeEv:'click',blocksX:1,blocksY:1,preset:'simpleFade',duration:800,easing:'linear',way:'lines',anim:'fade',pagination:true,banners:false,waitBannerAnimation:true,slideshow:false,progressBar:false,pauseOnHover:false,nextBu:false,prevBu:false,playBu:false,preFu:function(){var _=this,img=$(new Image())
...
_.pags.children().each(function(i){var li=$(this),a=$('a',li).bind(_.changeEv,function(){_.changeFu(i)
return false})}),_.pags=_.pags.find('li'),_.pags.eq(_.n).addClass(_.currCl)},startSlShFu:function(prog){var _=this
_.paused=false
_.prog=prog||0
...
banner.stop().css({right:-banner.width()-6}).animate({right:0},{duration:_.bannerDuration,easing:_.bannerEasing})},bannerHide:function(banner,_){banner.stop().animate({right:-banner.width()-6},{duration:_.bannerDuration,easing:_.bannerEasing})}},fromBottom:{bannerShow:function(banner,_){if(banner.css('left')=='auto')
banner.css('left',0)
...
banner.css('left',0)
banner.stop().css({top:-banner.height()}).animate({top:0},{duration:_.bannerDuration,easing:_.bannerEasing})},bannerHide:function(banner,_){banner.stop().animate({top:-banner.height()},{duration:_.bannerDuration,easing:_.bannerEasing})}},fade:{bannerShow:function(banner,_){if(banner.css('right')=='auto')
banner.css('right',0)
if(banner.css('top')=='auto')
banner.css('top',0)
banner.stop().css({opacity:0}).animate({opacity:1},{duration:_.bannerDuration,easing:_.bannerEasing})},bannerHide:function(banner,_){banner.stop().animate({opacity:0},{duration:_.bannerDuration,easing:_.bannerEasing})}}},numStatusChFu:function(){var _=this
